...
![Build README](https://github.com/davetang/learning_bash/actions/workflows/create_readme.yml/badge.svg)
...
A Bash _script_ or _program_ is simply a file that contains Bash commands. Your
`.bash_profile` and `.bashrc` files are Bash scripts. You can run a Bash script
using the `source my_script` command or using `bash my_script` (or by adding
`#!/usr/bin/env bash` to the start of your script and making your file
executable, i.e. `chmod 755 my_script`) but there is an important difference
between the two. Using `source` causes the commands in the script to be run as
if they were part of your login session, where as using `bash` will run the
script in a _subshell_. This means that a copy of the shell, which is a
subprocess of the parent, is invoked.
...
In Linux, `<`, `<<`, and `<<<` are redirection operators used for input handling.

* `<` (input redirection) - redirects input from a file to a command instead of reading from standard input.
...
* `<<` (here doc) - creates a multi-line input block that continues until it encounters a specified delimiter (often `EOF`).
...
Output redirection lets you control where command output goes. Standard output
(stdout) is file descriptor 1 and standard error (stderr) is file descriptor 2.

* `>` (overwrite) - redirects stdout to a file, creating or overwriting it.
...
* `2>&1` - redirects stderr to wherever stdout is going. This is useful for
  combining streams in a pipeline.
...
* `tee` - reads from stdin and writes to both stdout and a file simultaneously.
  This is useful for saving intermediate output while still passing it through a
  pipeline.
...
`-A`. Elements are referenced using square brackets not braces.
...
Quoting controls how Bash interprets special characters. There are three main
forms: single quotes, double quotes, and no quotes.

Single quotes preserve everything literally; no variable expansion or special
character interpretation occurs.
...
Double quotes allow variable expansion and command substitution but prevent word
...
Without quotes, Bash performs word splitting and globbing on the result. This can
...
Quoting is especially important when dealing with filenames that contain spaces
or special characters, and when passing arguments that should remain as a single
word.
...
* The `$0` variable is reserved for the function's name (but this doesn't seem
  like the case in the example below).
...
Use `local` to set local scoping for variables but this seems to be for use
with functions only.
...
Bash supports integer arithmetic using `$(( ))` for arithmetic expansion and
...
Use `(( ))` as a condition in `if` statements. It returns 0 (true) if the
expression is non-zero, and 1 (false) if the expression is zero.
...
For floating-point arithmetic, use `awk` or `bc` since Bash only supports
integers natively.
...
The older backtick syntax `` `command` `` is equivalent but harder to read and
nest, so `$()` is preferred (see [Best practices](#best-practices)).
...
Non-greedy deletion starting from string end; delete everything until matching
a `/` (including the `/`).
...
The `if` statement is the primary conditional construct in Bash. The basic syntax
is `if/then/elif/else/fi`.
...
Combine conditions using `&&` (and) and `||` (or) inside double brackets.
...
Short-circuit evaluation is a concise alternative for simple conditionals.
...
The `case` statement matches a value against patterns, similar to a switch
statement in other languages. Each pattern is terminated by `;;`.
...
Execute commands until a condition becomes true (the opposite of `while`).
...
Use `while read` to process a file line by line. This is preferred over `for`
loops for reading files because it handles whitespace correctly.
...
The `read` builtin reads a line from standard input and assigns it to variables.
...
Every command in Bash returns an exit code (also called exit status or return
code). By convention, 0 means success and any non-zero value indicates failure.
...
Inside a function, use `return` instead of `exit`. `return` exits the function
while `exit` terminates the entire script.
...
`${PIPESTATUS[@]}` to check the exit code of each command in the pipeline.
...
The `trap` command lets you execute commands when a script receives a signal or
exits. This is commonly used for cleanup tasks like removing temporary files.
...
* The `biotool` directory in this repository contains a script (also called
  `biotool`) that follows [Ten recommendations for creating usable
...
* A lot of Bash syntax is obscure, which minimises typing, but makes it hard to
  read. I highly recommend adding comments to your code.
...
* A side note about the difference between `()` and `$()`; the former is for grouping commands together in a subshell `(echo Hello; echo World)` and the latter is meant for command substitution where it makes it convenient to store command outputs as a variable `ME=$(whoami)`.
...
* If your script is getting big (over 100 lines), consider splitting it up into
  functions stored in different files.

* Lastly, if your code is getting too complicated, consider using another
  programming language. They all have their advantages and disadvantages, so
  try to pick one that suits your problem. Python is often a good choice for
